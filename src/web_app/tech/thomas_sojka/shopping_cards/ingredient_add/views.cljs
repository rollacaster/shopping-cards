(ns tech.thomas-sojka.shopping-cards.ingredient-add.views
  (:require
   [re-frame.core :refer [dispatch subscribe]]
   [reagent.core :as r]))

(defn new-ingredient []
   (dispatch [:query {:q '[:find ?name
                          :where
                          [_ :ingredient/category ?i]
                          [?i :db/ident ?name]]
                     :on-success [:ingredient/success-categories-load]}])
