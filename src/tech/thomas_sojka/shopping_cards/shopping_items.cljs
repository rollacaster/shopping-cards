...
(defn- sort-ingredients [sort-order ingredients]
...
(defn- sort-shopping-items [sort-order ingredients]
  (let [sort-order-map (into {} (map-indexed (fn [idx id] [id idx]) sort-order))]
    (sort-by (fn [shopping-item]
               (sort-order-map (:shopping-item/ingredient-id shopping-item)))
             ingredients)))

(defn- create-shopping-list [selected-items items]
  (->> items
...
(reg-event-fx :shopping-item/create
  ...
  (fn [{:keys [meals-without-shopping-list]} [_ {:keys [items selected-items]}]]
    {:firestore/add-docs {:path firestore-path
                          :id :shopping-item/id
                          :data (create-shopping-list selected-items items)
...
(reg-sub :shopping-item/possible-items
  ...
  (partial create-possible-shopping-items kitchen-order))

(reg-sub :shopping-entries
  (fn [db]
    (->> (:shopping-entries db)
         (remove (fn [{:keys [shopping-item/status]}] (= status :archive)))
         (sort-shopping-items penny-order))))
